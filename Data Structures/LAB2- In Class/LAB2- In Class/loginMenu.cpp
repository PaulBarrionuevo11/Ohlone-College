...
LoginMenu::LoginMenu() : Menu("Login Menu")
{
	...
	addOption(op2);
	addOption(op3);
	// Link all the trees in constructor to build the tree structure
	//MainMenu mainMenu;
	//menuTree = Tree<Menu>(*this); // Root Menu LOGIN MENU
	//menuTree.add_subtree(Tree<Menu>(mainMenu));
...
void LoginMenu::activate()
{
	...
	//while (currentNode)
	//{
	//	Menu& currentMenu = currentNode->getData(); // Current menu object
	//	char choice = currentMenu.getInput();  // Display and get user input
	//	
	//	switch (choice) {
	//	case USER_LOGIN:
	//		login();
